...
- 当前已迁移云台方向控制、自动巡航、预置点下拉切换、变倍、变焦、光圈、实时预览/回放 JPG 抓图、NVR 存储录像查询、开始回放、停止回放和录像下载；暂未迁移录像剪辑、OSD、设备维护等高级能力。
...
- 云台功能支持方向、自动、预置点下拉切换、变倍、变焦和光圈控制；预置点列表来自 `src/config/cameraConfig.js` 的 `presets` 配置。
...
6. 需要保存证据时，可在实时预览或录像回放窗口点击“抓图 JPG”；需要导出录像时，先选择录像片段，再点击“下载片段”或“按时间下载”。
